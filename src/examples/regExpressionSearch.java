...
public class regExpressionSearch {
    ...
    public static void main(String args[]){
        ...
        String pattern = ".*(4\\d{2}-?.*)";
        String pattern2 = "^(SsN is ).*";
        ...
        boolean isContain = content.contains("456-34-8778 ");
        System.out.println("The String contains SSN word? " + isContain);

        System.out.println("\nMatching case insensitive pattern");
        Pattern patternString = Pattern.compile(pattern2, Pattern.CASE_INSENSITIVE);
        ...
        System.out.println("\nExtracting pattern");
        Pattern pt = Pattern.compile("(Hunko|Gunko|Volodymyr)");
        Matcher mch = pt.matcher(content);
        while (mch.find()) {
            System.out.println("Name is: "+mch.group());
        }

        System.out.println("\nSubstitution");
        Pattern ptr = Pattern.compile("((SSN is )(\\d+-?\\d+-?\\d+))");
        ...
        while(match.find()) {
            ...
            match.appendReplacement(result, match.group(2)+"***-**-****");
